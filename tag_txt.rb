...
dir_name = "/media/mynewdrive/face_to_face"
#dir_name = "./test"
#h = Hash.new
sf = File.open("htable_stop_words")
pf = File.open("htable_punc_tag")

name = "/media/mynewdrive/POS_tag_face_to_face"
#pos_file = File.open("./corpus.txt", "r")
pos_file = File.open(name, "w")
#pos_file = File.open(name, "w")
...
=end
               pos_file.puts tagged_words[i].first + " NOUN"
...
=end
					pos_file.puts tagged_words[i].first + " VERB"
				when "JJ", "JJR", "JJS"
					#puts "adj"
					#tagged_words[i] = [tagged_words[i].first, "ADJ"]
					pos_file.puts tagged_words[i].first + " ADJ"
				when "RB", "RBR", "RBS"
					#puts "adv"
					#tagged_words[i] = [tagged_words[i].first, "ADV"]
					pos_file.puts tagged_words[i].first + " ADV"
				else
					#puts "crap"
					next
				end
